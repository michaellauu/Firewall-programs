    ...
    if ip is not None:
      ...
      if switch_id == 1:
        ...
        if ip.dstip == "123.45.67.89":
          ...
          if ip.srcip == "123.45.67.89" and ICMP is not None: #if ICMP traffic from h4 drop
            return
          else:
            msg.actions.append(of.ofp_action_output(port = 1))
            msg.data = packet_in
          ...
          if ip.srcip == "123.45.67.89" and ICMP is not None: #if ICMP traffic from h4 drop
            ...
            msg.data = packet_in
            self.connection.send(msg)
        elif ip.dstip == "10.5.5.50" and ip.srcip != "123.45.67.89": #block all ip from h4
          msg.actions.append(of.ofp_action_output(port = 5))
          msg.data = packet_in
          self.connection.send(msg)
        else: #if dstip isn't stated, drop packet
          return
   
    else: #flood all other traffic
      	msg.match = of.ofp_match.from_packet(packet)
